...
create trigger embed_document_sections
  ...
  referencing new table as inserted
  -- ensures that the trigger is executed ones for each batch vs once for each row
  for each statement
  -- "content" is theinput column that contains the chunk of text to be embedded
  -- "embedding" is the destinationcolumn that will store the embedding vector
  -- batch_size (third param) is the number of records to include in each edge function call
